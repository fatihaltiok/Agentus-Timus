...
DEFAULT_LOCK = PROJECT_ROOT / "logs" / "timus_restart.lock"
...
def _release_lock(lock_file: Path, request_id: str) -> None:
    payload = _read_json(lock_file) or {}
    owner = str(payload.get("request_id") or "")
...
def _run_supervised_restart(
    ...
    print(f"[restart-supervisor] start mode={mode} request_id={request_id}", flush=True)
    ...
    try:
        ...
        print(f"[restart-supervisor] timeout mode={mode}: {exc}", flush=True)
        return 1
        ...
        print(f"[restart-supervisor] exception mode={mode}: {exc}", flush=True)
        return 1
    ...
    print(f"[restart-supervisor] finish mode={mode} rc={proc.returncode}", flush=True)
...
def main(argv: list[str] | None = None) -> int:
    ...
    request_id = args.request_id or f"manual-{os.getpid()}"
    try:
        return _run_supervised_restart(args.mode, script_path, status_file, lock_file, request_id)
    finally:
        _release_lock(lock_file, request_id)
