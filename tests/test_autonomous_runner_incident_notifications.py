...
@pytest.mark.asyncio
async def test_improvement_task_retry_is_not_notified_as_terminal_failure(monkeypatch, tmp_path: Path) -> None:
    queue = TaskQueue(db_path=tmp_path / "task_queue.db")
    task_id = queue.add(
        description="[PHASE E E3.2] Improvement Hardening Task fuer tool:retry_screen_contract.",
        target_agent="development",
        max_retries=3,
        metadata=json.dumps({"source": "improvement_task_bridge"}, ensure_ascii=True),
    )
    ...
    assert deliveries == {"telegram": 0, "email": 0}
    event_types = [item["event_type"] for item in events]
    assert event_types == [
        "task_execution_started",
        "task_route_selected",
