...
from orchestration.delegation_parity_harness import summarize_delegation_parity_results


@given(passed_flags=st.lists(st.booleans(), min_size=0, max_size=20))
def test_delegation_parity_summary_counts_are_consistent(passed_flags: list[bool]) -> None:
    rows = [
        {"scenario_id": f"scenario_{index}", "passed": flag}
        for index, flag in enumerate(passed_flags)
    ]
    summary = summarize_delegation_parity_results(rows)
