...
@deal.post(lambda r: isinstance(r["current_capabilities"], list))
@deal.post(lambda r: isinstance(r["partial_capabilities"], list))
@deal.post(lambda r: isinstance(r["planned_capabilities"], list))
@deal.post(lambda r: isinstance(r["blocked_capabilities"], list))
@deal.post(lambda r: isinstance(r["confidence_bounds"], list))
@deal.post(lambda r: isinstance(r["autonomy_limits"], list))
...
def test_hypothesis_meta_self_state_shape(
    ...
    assert all(isinstance(item, str) for item in result["current_capabilities"])
    assert all(isinstance(item, str) for item in result["partial_capabilities"])
    assert all(isinstance(item, str) for item in result["planned_capabilities"])
    assert all(isinstance(item, str) for item in result["blocked_capabilities"])
    assert all({"area", "level", "reason"} <= set(item.keys()) for item in result["confidence_bounds"])
    assert all({"limit", "state", "reason"} <= set(item.keys()) for item in result["autonomy_limits"])
