...
let routeMapDirectionsService = null;
let routeMapClientPathCache = { key: "", path: [], promise: null };
...
let routeMapViewportPinned = false;
let routeMapSuppressViewportTracking = false;
...
function _routeMapPathCoordinates(route) {
  const rawPoints = Array.isArray((route || {}).path_coordinates) ? route.path_coordinates : [];
  const points = [];
  rawPoints.forEach(value => {
    const coords = _extractRouteMapCoordinates(value);
    if (!coords) return;
    const previous = points[points.length - 1];
    if (
      previous &&
      Math.abs(previous.lat - coords.lat) < 0.0000001 &&
      Math.abs(previous.lng - coords.lng) < 0.0000001
    ) {
      return;
    }
    points.push(coords);
...
function _routeMapHasInteractiveGeometry(route) {
  const origin = _extractRouteMapCoordinates((route || {}).start_coordinates || (route || {}).origin || {});
  const destination = _extractRouteMapCoordinates((route || {}).end_coordinates || {});
  ...
  return Boolean(
    ...
    _routeMapPathCoordinates(route).length > 1 ||
    (origin && (destination || destinationLabel))
  );
}

function _routeMapDirectionsCacheKey(route) {
...
function _routeMapTravelMode(maps, value) {
  const normalized = String(value || "driving").trim().toLowerCase();
...
function _routeMapNormalizeLatLng(value) {
  if (!value) return null;
  if (typeof value.lat === "function" && typeof value.lng === "function") {
    return { lat: value.lat(), lng: value.lng() };
  }
  return _extractRouteMapCoordinates(value);
}

async function _routeMapComputeBrowserPath(maps, route) {
  const cacheKey = _routeMapDirectionsCacheKey(route);
  if (!cacheKey || !maps?.DirectionsService) return [];
  if (routeMapClientPathCache.key === cacheKey) {
    if (Array.isArray(routeMapClientPathCache.path) && routeMapClientPathCache.path.length) {
      return routeMapClientPathCache.path;
    }
    if (routeMapClientPathCache.promise) {
      try {
        return await routeMapClientPathCache.promise;
      } catch {
        return [];
      }
    }
  ...
  routeMapDirectionsService = routeMapDirectionsService || new maps.DirectionsService();
  const request = {
    origin,
    destination: destination || destinationLabel,
    travelMode: _routeMapTravelMode(maps, route.travel_mode),
  ...
  const promise = new Promise((resolve) => {
    routeMapDirectionsService.route(request, (response, status) => {
      ...
      resolve(
        overviewPath
          .map(_routeMapNormalizeLatLng)
          .filter(Boolean),
      );
    });
  });
  routeMapClientPathCache = { key: cacheKey, path: [], promise };
  const path = await promise;
  routeMapClientPathCache = { key: cacheKey, path, promise: null };
  return path;
}

async function _routeMapDecodedPath(maps, route) {
  ...
  const browserPath = await _routeMapComputeBrowserPath(maps, route);
  if (browserPath.length) return browserPath;
  return _routeMapPathCoordinates(route);
}
...
function _persistRouteMapFollowPreference() {
  try { window.localStorage.setItem("timusRouteMapFollowEnabled", routeMapFollowEnabled ? "1" : "0"); } catch {}
}

function _withRouteMapViewportMutation(callback) {
  routeMapSuppressViewportTracking = true;
  try {
    callback();
  } finally {
    const mapsEvent = window.google?.maps?.event;
    if (routeMapInstance && mapsEvent?.addListenerOnce) {
      mapsEvent.addListenerOnce(routeMapInstance, "idle", () => {
        routeMapSuppressViewportTracking = false;
      });
    } else {
      window.setTimeout(() => {
        routeMapSuppressViewportTracking = false;
...
function _handleRouteMapManualViewport(reason) {
  if (routeMapSuppressViewportTracking) return;
  routeMapViewportPinned = true;
  if (routeMapFollowEnabled) {
    routeMapFollowEnabled = false;
    _persistRouteMapFollowPreference();
    _setRouteMapModeNote(`Follow pausiert (${reason || "manuelle Kartenbewegung"})`);
  }
  _syncRouteMapModeControls();
}

function _updateRouteMapCaption(route, mode, { fallback = false } = {}) {
  const captionEl = document.getElementById("routeMapCaption");
  if (!captionEl) return;
  if (!route || !route.has_route) {
    captionEl.textContent = "Interaktive Karte nutzt Google Maps JS, faellt aber automatisch auf die serverseitige Kartenansicht zurueck.";
    return;
  }
  const provider = route.source_provider || route.engine || "route";
  if (mode === "interactive" && !fallback) {
    captionEl.textContent = `Interaktive Google-Maps-Karte mit Zoom, Pan und Live-Follow. Provider: ${provider}.`;
    return;
  }
  captionEl.textContent = `Serverseitige PNG-Karte der aktiven Route. Provider: ${provider}. Nutze 'Interaktiv' fuer Zoom und Pan.`;
}
...
function setRouteMapMode(mode) {
  ...
  if (routeMapMode === "interactive" && !routeMapFollowEnabled) {
    routeMapViewportPinned = true;
  }
  _syncRouteMapModeControls();
...
async function _renderInteractiveRouteMap(route, forceRefresh) {
  const config = await loadRouteMapConfig(false);
  if (!config.interactive_available || !route || !route.has_route || !_routeMapHasInteractiveGeometry(route)) return false;
  ...
  if (!routeMapInstance) {
    ...
    maps.event.addListener(routeMapInstance, "dragstart", () => {
      _handleRouteMapManualViewport("manuelle Verschiebung");
      container.style.cursor = "grabbing";
    });
    maps.event.addListener(routeMapInstance, "dragend", () => {
      container.style.cursor = "grab";
    });
    maps.event.addListener(routeMapInstance, "zoom_changed", () => {
      _handleRouteMapManualViewport("Zoom");
    });
  }

  const decodedPath = await _routeMapDecodedPath(maps, route);
  ...
  const shouldFitBounds = forceRefresh || highlightedStep || !routeMapViewportPinned;
  ...
  _updateRouteMapCaption(route, "interactive");
  _syncRouteMapStepSelection();
  return true;
}
...
async function _renderRouteMap(route, forceRefresh) {
  ...
  if (!route || !route.has_route) {
    ...
    _updateRouteMapCaption(route, "static");
    _syncRouteMapModeControls();
    return;
  }
  const effectiveMode = routeMapMode === "interactive" && config.interactive_available && _routeMapHasInteractiveGeometry(route)
    ? "interactive"
    : "static";
  if (effectiveMode === "interactive") {
    const rendered = await _renderInteractiveRouteMap(route, forceRefresh);
    if (rendered) {
      _syncRouteMapModeControls();
      return;
    }
    _setRouteMapModeNote("Interaktive Karte fehlgeschlagen — Static Fallback aktiv");
    _updateRouteMapCaption(route, "static", { fallback: true });
  }
  _clearInteractiveRouteMap();
  _setRouteMapImage(route, forceRefresh);
  _updateRouteMapCaption(route, "static");
...
const FLOW_GROUPS = {
  ...
  memory: {
    ...
    summaryEdges: ["e-M-MEMORY_SUM", "e-MEMORY_SUM-CAP"],
    children: ["MM", "QDR", "WAL", "MAG", "CUR", "RFT", "SE", "SEA", "SED", "SEP", "CE", "CEQ", "CEG", "CEF"],
  },
  autonomy: {
    label: "Autonomy",
    ...
    children: ["SME", "SM1", "SM2", "SM3", "SM4", "SM5", "SM6", "SM7", "RUN", "SG", "G1", "G4", "G5", "G6", "G7", "G8", "G9", "G10", "G12", "G13", "AMBC"],
  },
};
...
const FLOW_ALIAS_NODE_IDS = {
  ...
  session: "CAP",
  capsule: "CAP",
  "conversation capsule": "CAP",
  "topic recall": "CAP",
  "matched_reply_points": "CAP",
  agent_class_map: "A",
  agent_classmap: "A",
  "13 agenten": "A",
  agentregistry: "AR",
  ...
  fanout: "ARP",
  "fan out": "ARP",
  "memoryaccessguard": "MAG",
  "resultaggregator": "ARP",
  result_aggregator: "ARP",
  metaagent: "META",
  "meta agent": "META",
  recipes: "MR",
  handoffs: "MH",
  ...
  gatekeeper: "CEG",
  feedbackengine: "CEF",
  selfmodification: "SME",
  "self modification": "SME",
  canary: "SM5",
  rollback: "SM5",
  autonomous_apply: "SM7",
...
const FLOW_KEYWORD_GROUPS = [
  { nodeIds: ["AND", "RP", "CU"], terms: ["android", "mobile console", "canvas ui", "reverse proxy", "caddy", "gps", "accessibility"] },
  { nodeIds: ["D"], terms: ["sanitize", "sanitizing", "query sanitize", "intent", "routing llm", "classifier", "policy", "guardrail", "lane"] },
  { nodeIds: ["CAP"], terms: ["session", "context", "capsule", "topic recall", "matched_reply_points", "follow-up"] },
  { nodeIds: ["AR", "ARD"], terms: ["delegate", "delegation"] },
  { nodeIds: ["ARD"], terms: ["retry", "backoff", "partial", "loop", "max_depth"] },
  { nodeIds: ["ARP"], terms: ["parallel", "fan-out", "fan out", "fan-in", "fan in", "gather", "aggregator"] },
  { nodeIds: ["MAG"], terms: ["memoryaccessguard", "contextvar", "thread-safe", "thread safe"] },
  { nodeIds: ["META", "MR", "MH", "MS", "ML", "MEG"], terms: ["meta", "recipe", "handoff", "meta_self_state", "outcome", "evidence gate", "belegt"] },
  { nodeIds: ["B", "BW", "BR"], terms: ["working memory", "prompt prefix", "soul-prefix", "soul prefix", "blackboard", "dynamictoolmixin"] },
  { nodeIds: ["M"], terms: ["mcp", "json-rpc", "jsonrpc", "tool", "fastapi"] },
  { nodeIds: ["FH"], terms: ["visualnemotron", "florence", "paddleocr", "vision", "plan-then-execute"] },
  { nodeIds: ["VC", "VW", "VT"], terms: ["voice", "listen", "speak", "whisper", "tts", "stt", "playback"] },
  { nodeIds: ["CF"], terms: ["file api", "upload", "download", "recent files"] },
  { nodeIds: ["LOC"], terms: ["location", "gps", "nearby", "reverse geocoding", "serpapi google maps"] },
  { nodeIds: ["SYS"], terms: ["systemagent", "system agent", "monitoring"] },
  { nodeIds: ["SH"], terms: ["shellagent", "shell agent", "bash", "terminal"] },
  { nodeIds: ["DR", "DRY", "DRI", "DRC"], terms: ["deep research", "youtube", "weasyprint", "claim", "evidence", "verdict", "partial_research"] },
  { nodeIds: ["SME", "SM1", "SM2", "SM3", "SM4", "SM5", "SM6", "SM7"], terms: ["self modification", "change policy", "risk classifier", "patch pipeline", "verification gate", "canary", "rollback", "change memory", "autonomous apply"] },
  { nodeIds: ["MM", "QDR", "WAL", "MAG", "CUR", "RFT"], terms: ["memory", "sqlite", "wal", "qdrant", "semantic recall", "curator", "reflection"] },
  { nodeIds: ["SE", "SEA", "SED", "SEP"], terms: ["soul", "apply_drift", "risk_appetite", "system prompt prefix"] },
  { nodeIds: ["CE", "CEQ", "CEG", "CEF"], terms: ["curiosity", "query", "gatekeeper", "feedback", "telegram push"] },
  { nodeIds: ["RUN", "SG", "G1", "G4", "G5", "G6", "G7", "G8", "G9", "G10", "G12", "G13", "AMBC"], terms: ["autonomous_runner", "stabilization", "goalgenerator", "selfhealing", "scorecard", "sessionreflection", "blackboard", "trigger", "goal queue", "self improvement", "email autonomy", "tool generator", "ambient context"] },
];
...
function buildArchitectureFlowElements() {
  const nodes = [
    flowNodeSpec("GROUP_VOICE", "Voice", "voice", { lane: "Group", isGroup: true, groupKey: "voice", zoneMinW: 620, zoneMinH: 280 }),
    flowNodeSpec("GROUP_MEMORY", "Memory", "memory", { lane: "Group", isGroup: true, groupKey: "memory", zoneMinW: 1180, zoneMinH: 860 }),
    flowNodeSpec("GROUP_AUTONOMY", "Autonomy", "autonomy", { lane: "Group", isGroup: true, groupKey: "autonomy", zoneMinW: 1380, zoneMinH: 560 }),
    flowNodeSpec("U", "User Input\nCLI / Telegram / Terminal", "entry", { lane: "Input", description: "Primaere Einstiegspunkte fuer Timus." }),
    flowNodeSpec("AND", "Android App\nKotlin - Chat - Voice - GPS\nAccessibilityService", "entry", { lane: "Input", h: 78 }),
    flowNodeSpec("RP", "Caddy Reverse Proxy\nHTTPS + Auth\nconsole.fatih-altiok.com", "external", { lane: "Input", h: 78 }),
    flowNodeSpec("CU", "Canvas UI / Mobile Console\nChat - Status - Dateien - Voice", "runtime", { lane: "Input", h: 72, w: 220 }),
    flowNodeSpec("D", "main_dispatcher.py\nQuery-Sanitizing - Intent-LLM\nPolicy Gate - Lane+Session\nFollow-up Resolver - Topic-Recall", "dispatch", { lane: "Dispatch", description: "Zentraler Router fuer Queries.", h: 92, w: 250 }),
    flowNodeSpec("CAP", "Conversation Capsule\nSession-State - last_agent\nmatched_reply_points\ntopic_recall inject", "memory", { lane: "Dispatch", h: 84, w: 220 }),
    flowNodeSpec("A", "AGENT_CLASS_MAP\n13 Agenten", "agent", { lane: "Agents", description: "Gemeinsamer Einstieg in die Agentenklasse." }),
    flowNodeSpec("AR", "AgentRegistry\nAgentResult - delegate/parallel\nBlackboard-Auto-Write", "agent", { lane: "Delegation", h: 78 }),
    flowNodeSpec("ARD", "delegate - sequenziell\nasyncio.wait_for 120s\nRetry - Partial - MAX_DEPTH 3", "agent", { lane: "Delegation", h: 78 }),
    flowNodeSpec("ARP", "delegate_parallel - Fan-Out\nasyncio.gather + Semaphore 10\nFan-In ResultAggregator", "agent", { lane: "Delegation", h: 78 }),
    flowNodeSpec("META", "MetaAgent R1/R2\norchestration/meta_orchestration.py", "agent", { lane: "Agents", h: 72 }),
    flowNodeSpec("MR", "Rezepte + Alternativrezepte\nyoutube_search_then_visual\nweb_research_only u.a.", "agent", { lane: "Agents", h: 84 }),
    flowNodeSpec("MH", "Strukturierte Handoffs\ngoal - expected_output\nsuccess_signal - constraints", "agent", { lane: "Agents", h: 78 }),
    flowNodeSpec("MS", "meta_self_state.py\nBudget - Breaker - Quarantaene\nbekannte schlechte Muster", "agent", { lane: "Agents", h: 84 }),
    flowNodeSpec("ML", "Outcome-Lernen\nsite_recipe - task_type\nkonservative Rezeptwahl", "agent", { lane: "Agents", h: 78 }),
    flowNodeSpec("MEG", "Evidence Gate\nSelbstdiagnose nur mit\nbelegten Daten\n[BELEGT]/[TEILWEISE]/[NICHT BELEGT]", "system", { lane: "Agents", h: 92 }),
    flowNodeSpec("B", "agent/base_agent.py\nDynamicToolMixin", "runtime", { lane: "Runtime" }),
    flowNodeSpec("BW", "Working Memory + Soul-Prefix", "runtime", { lane: "Runtime" }),
    flowNodeSpec("BR", "Blackboard-Kontext inject", "runtime", { lane: "Runtime" }),
    ...
    flowNodeSpec("VC", "Voice REST API\n/voice/status|listen|stop|speak|synthesize", "voice", { lane: "Voice", h: 72, parent: "GROUP_VOICE" }),
    flowNodeSpec("VW", "Faster-Whisper STT", "voice", { lane: "Voice", parent: "GROUP_VOICE" }),
    flowNodeSpec("VT", "Inworld.AI TTS\nBase64-MP3 + Browser Playback", "voice", { lane: "Voice", parent: "GROUP_VOICE" }),
    flowNodeSpec("VOICE_SUM", "Voice Cluster\nAndroid + Canvas + REST", "voice", { lane: "Voice", isSummary: true, groupKey: "voice", w: 196, h: 74, description: "Zusammenfassung des Voice-Subsystems." }),
    flowNodeSpec("CF", "Console File API\n/upload - /files/recent - /files/download", "runtime", { lane: "Tools", h: 78 }),
    flowNodeSpec("LOC", "Location API\n/location/update - /location/nearby\nReverse-Geocoding\nSerpApi Google Maps", "runtime", { lane: "Tools", h: 92, w: 228 }),
    flowNodeSpec("SYS", "SystemAgent S.Y.S.\nread-only - Logs - Stats\nAnti-Halluzination Gate", "system", { lane: "Agents", h: 78 }),
    flowNodeSpec("SH", "ShellAgent v2\n5-Schicht-Policy\nSystem-Kontext-Injektion", "system", { lane: "Agents", h: 72 }),
    flowNodeSpec("DR", "Deep Research v8.1\nEvidence Engine", "runtime", { lane: "Tools", h: 72 }),
    flowNodeSpec("DRY", "YouTubeResearcher\nDataForSEO live-Mode\nStandard->Live Fallback\nSerpApi YouTube", "runtime", { lane: "Tools", h: 92 }),
    flowNodeSpec("DRI", "ImageCollector + PDF\nWeasyPrint A4", "runtime", { lane: "Tools", h: 72 }),
    flowNodeSpec("DRC", "Claim->Evidence->Verdict\npartial_research Guardrail", "runtime", { lane: "Tools", h: 72 }),
    flowNodeSpec("E", "Externe Systeme\nPyAutoGUI / Playwright / APIs", "external", { lane: "External" }),
    flowNodeSpec("SME", "Self-Modification\nSM1-SM7\nself_modifier_engine.py", "autonomy", { lane: "Autonomy", h: 78, parent: "GROUP_AUTONOMY" }),
    flowNodeSpec("SM1", "SM1 Change Policy\nZonen - Typen - Testpflicht", "autonomy", { lane: "Autonomy", parent: "GROUP_AUTONOMY" }),
    flowNodeSpec("SM2", "SM2 Risk Classifier\nRisikostufe + Begruendung", "autonomy", { lane: "Autonomy", parent: "GROUP_AUTONOMY" }),
    flowNodeSpec("SM3", "SM3 Patch-Pipeline\nisolierter Workspace", "autonomy", { lane: "Autonomy", parent: "GROUP_AUTONOMY" }),
    flowNodeSpec("SM4", "SM4 Verification Gate\npy_compile - Tests - Syntax", "autonomy", { lane: "Autonomy", parent: "GROUP_AUTONOMY" }),
    flowNodeSpec("SM5", "SM5 Canary + Rollback\nautomatisches Revert", "autonomy", { lane: "Autonomy", parent: "GROUP_AUTONOMY" }),
    flowNodeSpec("SM6", "SM6 Change Memory\nSQLite-Ledger\nOutcome - Regression", "autonomy", { lane: "Autonomy", h: 72, parent: "GROUP_AUTONOMY" }),
    flowNodeSpec("SM7", "SM7 Autonomous Apply\nController\nPolicy+Risk+Verify+Canary", "autonomy", { lane: "Autonomy", h: 78, parent: "GROUP_AUTONOMY" }),
    flowNodeSpec("MM", "memory/memory_system.py\nMemory + WAL", "memory", { lane: "Memory", parent: "GROUP_MEMORY" }),
    flowNodeSpec("QDR", "Qdrant\nsemantischer Chat-Recall\nHit Rate@k - wrong recall\nproduktiver Default", "memory", { lane: "Memory", h: 92, parent: "GROUP_MEMORY" }),
    flowNodeSpec("WAL", "SQLite WAL\ninteraction_events\nGoals - Incidents - Analytics", "storage", { lane: "Memory", h: 78, parent: "GROUP_MEMORY" }),
    flowNodeSpec("MAG", "MemoryAccessGuard\nContextVar", "memory", { lane: "Memory", parent: "GROUP_MEMORY" }),
    flowNodeSpec("CUR", "Nemotron-Kurator\nAuto-Summarize alle 20", "memory", { lane: "Memory", parent: "GROUP_MEMORY" }),
    flowNodeSpec("RFT", "Reflection -> soul_engine.apply_drift", "memory", { lane: "Memory", h: 60, parent: "GROUP_MEMORY" }),
    flowNodeSpec("SE", "SoulEngine\nmemory/soul_engine.py", "soul", { lane: "Soul", parent: "GROUP_MEMORY" }),
    flowNodeSpec("SEA", "5 Achsen\nconfidence formality humor\nverbosity risk_appetite", "soul", { lane: "Soul", h: 72, parent: "GROUP_MEMORY" }),
    flowNodeSpec("SED", "apply_drift - Decay taeglich", "soul", { lane: "Soul", h: 60, parent: "GROUP_MEMORY" }),
    flowNodeSpec("SEP", "get_system_prompt_prefix\ndynamisches Prompt-Fragment", "soul", { lane: "Soul", parent: "GROUP_MEMORY" }),
    flowNodeSpec("CE", "CuriosityEngine\norchestration/curiosity_engine.py", "curiosity", { lane: "Curiosity", parent: "GROUP_MEMORY" }),
    flowNodeSpec("CEQ", "LLM Query-Gen + DataForSEO", "curiosity", { lane: "Curiosity", parent: "GROUP_MEMORY" }),
    flowNodeSpec("CEG", "Gatekeeper >=7 -> Telegram Push", "curiosity", { lane: "Curiosity", h: 72, parent: "GROUP_MEMORY" }),
    flowNodeSpec("CEF", "FeedbackEngine M16\n+/ - / ? -> Soul-Hook-Gewichtung", "curiosity", { lane: "Curiosity", h: 72, parent: "GROUP_MEMORY" }),
    flowNodeSpec("MEMORY_SUM", "Memory Cluster\nQdrant + WAL + Soul + Curiosity", "memory", { lane: "Memory", isSummary: true, groupKey: "memory", w: 220, h: 78, description: "Zusammenfassung von Memory, Soul und Curiosity." }),
    flowNodeSpec("RUN", "autonomous_runner.py\nAutonomie-Loop v4.12", "autonomy", { lane: "Autonomy", parent: "GROUP_AUTONOMY" }),
    flowNodeSpec("SG", "Self-Stabilization Gate S1-S6\nDedupe - Recovery-Leiter\nQuarantaene - Resource-Guard\nIncident-Memory - Stability-Gate", "autonomy", { lane: "Autonomy", h: 92, parent: "GROUP_AUTONOMY" }),
    flowNodeSpec("G1", "GoalGenerator M1 + LongTermPlanner M2", "autonomy", { lane: "Autonomy", h: 60, parent: "GROUP_AUTONOMY" }),
    ...
    flowNodeSpec("G6", "SessionReflection M8\nIdle - LLM-Reflexion - Pattern", "autonomy", { lane: "Autonomy", h: 72, parent: "GROUP_AUTONOMY" }),
    flowNodeSpec("G7", "AgentBlackboard M9\nTTL Shared Memory", "autonomy", { lane: "Autonomy", h: 60, parent: "GROUP_AUTONOMY" }),
    flowNodeSpec("G8", "ProactiveTriggers M10\nMorgen + Abend-Routinen", "autonomy", { lane: "Autonomy", h: 60, parent: "GROUP_AUTONOMY" }),
    flowNodeSpec("G9", "GoalQueueManager M11\nHierarchische Ziele", "autonomy", { lane: "Autonomy", h: 60, parent: "GROUP_AUTONOMY" }),
    flowNodeSpec("G10", "SelfImprovementEngine M12\nTool-/Routing-Analytics", "autonomy", { lane: "Autonomy", h: 60, parent: "GROUP_AUTONOMY" }),
    flowNodeSpec("G12", "EmailAutonomyEngine M14\nWhitelist+Confidence+SMTP", "autonomy", { lane: "Autonomy", h: 60, parent: "GROUP_AUTONOMY" }),
    flowNodeSpec("G13", "ToolGeneratorEngine M13\nAST-Check+Review+importlib", "autonomy", { lane: "Autonomy", h: 60, parent: "GROUP_AUTONOMY" }),
    flowNodeSpec("AMBC", "AmbientContextEngine M15\nSignalScorer - Policy-Layer\nAudit-Log", "autonomy", { lane: "Autonomy", h: 78, parent: "GROUP_AUTONOMY" }),
    flowNodeSpec("AUTONOMY_SUM", "Autonomy Cluster\nStabilization + Goals + Improvement", "autonomy", { lane: "Autonomy", isSummary: true, groupKey: "autonomy", w: 232, h: 78, description: "Zusammenfassung des Autonomy-Subsystems." }),
  ];
  ...
  const edges = [
    ...
    flowEdgeSpec("CU", "D"),
    flowEdgeSpec("D", "CAP"),
    flowEdgeSpec("CAP", "D", { dashed: true, label: "Kontext" }),
    flowEdgeSpec("D", "A"),
    flowEdgeSpec("A", "AR"),
    flowEdgeSpec("AR", "ARD"),
    flowEdgeSpec("AR", "ARP"),
    flowEdgeSpec("A", "META"),
    flowEdgeSpec("META", "MR"),
    flowEdgeSpec("META", "MH"),
    flowEdgeSpec("META", "MS"),
    flowEdgeSpec("META", "ML"),
    flowEdgeSpec("META", "MEG"),
    flowEdgeSpec("A", "B"),
    flowEdgeSpec("B", "BW"),
    flowEdgeSpec("B", "BR"),
    flowEdgeSpec("B", "M"),
    ...
    flowEdgeSpec("VC", "VW"),
    flowEdgeSpec("VC", "VT"),
    flowEdgeSpec("AND", "VC", { dashed: true, label: "/voice/transcribe / /voice/synthesize" }),
    flowEdgeSpec("M", "CF"),
    flowEdgeSpec("CU", "CF"),
    flowEdgeSpec("M", "LOC"),
    flowEdgeSpec("AND", "LOC", { dashed: true, label: "GPS lat/lon" }),
    flowEdgeSpec("M", "SYS"),
    flowEdgeSpec("M", "SH"),
    flowEdgeSpec("M", "DR"),
    flowEdgeSpec("DR", "DRY"),
    ...
    flowEdgeSpec("M", "E"),
    flowEdgeSpec("M", "SME"),
    flowEdgeSpec("SME", "SM1"),
    flowEdgeSpec("SME", "SM2"),
    flowEdgeSpec("SME", "SM3"),
    flowEdgeSpec("SME", "SM4"),
    flowEdgeSpec("SME", "SM5"),
    flowEdgeSpec("SME", "SM6"),
    flowEdgeSpec("SME", "SM7"),
    flowEdgeSpec("M", "MM"),
    flowEdgeSpec("MM", "QDR"),
    flowEdgeSpec("MM", "WAL"),
    flowEdgeSpec("MM", "MAG"),
    ...
    flowEdgeSpec("ARP", "MAG", { dashed: true, label: "read-only" }),
    flowEdgeSpec("QDR", "CAP", { dashed: true, label: "semantischer Recall" }),
    flowEdgeSpec("D", "RUN"),
    flowEdgeSpec("RUN", "SG"),
    flowEdgeSpec("RUN", "G1"),
    flowEdgeSpec("RUN", "G4"),
    ...
    flowEdgeSpec("MEMORY_SUM", "CAP", { id: "e-MEMORY_SUM-CAP", dashed: true, color: "#4a5e8a", opacity: 0.55 }),
