...
@method
async def voice_listen(duration: Optional[float] = None) -> Union[Success, Error]:
    """
    Hört zu und transkribiert Sprache zu Text.
...
@method
async def voice_speak(text: str, voice: Optional[str] = None) -> Union[Success, Error]:
    """
    Spricht Text mit ElevenLabs.
...
@method
async def voice_set_voice(voice_name: str) -> Union[Success, Error]:
    """
    ...
    if success:
        ...
        return Error(code=-32001, message=f"Unbekannte Stimme: {voice_name}. Verfügbar: {list(VOICES.keys())}")


@method
async def voice_list_voices() -> Union[Success, Error]:
    """
...
@method
async def voice_set_language(language: str) -> Union[Success, Error]:
    """
...
@method
async def voice_chat_turn(timeout: float = 10.0) -> Union[Success, Error]:
    """
...
@method
async def voice_initialize() -> Union[Success, Error]:
    """
...
register_tool("voice_listen", voice_listen)
register_tool("voice_speak", voice_speak)
register_tool("voice_set_voice", voice_set_voice)
register_tool("voice_list_voices", voice_list_voices)
register_tool("voice_set_language", voice_set_language)
register_tool("voice_chat_turn", voice_chat_turn)
register_tool("voice_initialize", voice_initialize)

log.info("✅ Voice Tool v1.0 registriert")
log.info(f"   Whisper: {WHISPER_MODEL} auf {WHISPER_DEVICE}")
log.info(f"   ElevenLabs: {'✅' if ELEVENLABS_API_KEY else '❌'}")
log.info("   Tools: voice_listen, voice_speak, voice_set_voice, voice_chat_turn")
