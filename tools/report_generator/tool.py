...
@tool(
    name="generate_report_from_session",
    description="Erstellt einen umfassenden Bericht aus einer abgeschlossenen Deep-Research-Session.",
    parameters=[
        P("session_id", "string", "Die ID der Recherche-Session, aus der der Bericht erstellt werden soll.", required=True),
        P("query", "string", "Die urspruengliche Nutzeranfrage fuer den Kontext.", required=True),
        P("report_format", "string", "Gewuenschtes Format (markdown, html, json).", required=False, default="markdown"),
        P("language", "string", "Sprache des Berichts (de oder en).", required=False, default="de"),
    ],
...
async def generate_report_from_session(
    ...
    try:
        ...
        if not session:
            raise Exception(f"Recherche-Session mit ID '{session_id}' nicht gefunden.")
        ...
        if report_format.lower() == "markdown":
            ...
            raise Exception(f"Ungültiges Format '{report_format}'.")
